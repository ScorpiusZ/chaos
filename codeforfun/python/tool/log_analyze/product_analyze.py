...
def product_statics(datetime,limit):
    ...
    print pr_format.format('date','product_id','product_pv','cart_count','order_count')
    for product_id in product_ids:
        print pr_format.format(datetime,id_util.decode_product(product_id),result.get(product_id,0),\
                cart_product_counts.get(product_id,0),\
                order_product_counts.get(product_id,0))
    print all_pr_format.format('sum',len(result.keys()),sum(result),\
